...
const ViewPaste = (props) => {
	...
	const getPaste = async () => {
		const { data, response } = await props.fetchPaste({
			name: name,
		});
		if (response.status === 401) {
			setPasswordArea(
				<form className="paste-password" onSubmit={handlePasswordSubmit}>
					<h1>پیست محافظت شده</h1>
					<p>
						پیست مورد نظر شما با پسورد محافظت شده است، لطفا جهت دیدن پیست پسورد
						را وارد نمایید
					</p>
					<div className="field">
						<input
							placeholder="پســــورد"
							type="password"
							onChange={(e) => {
					...
					<div className="submit">
						<input type="submit" value="دیدن پیست" />
					</div>
				</form>
			);
		} else if (response.status === 404) {
			setPasswordArea(<h1>not found</h1>);
		} else {
			setLiked(data.liked);
			setPaste(data);
			const fetchedComments = await props.fetchComments(data.paste.id);
			setComments(fetchedComments);
		}
	};
	const handlePasswordSubmit = async (e) => {
		e.preventDefault();
		let password = document.getElementById("paste-password").value;
		const { data, response } = await props.fetchPaste({
			name: name,
			password: password,
		});
		if (response.status === 401 || password === "") {
			document.getElementById("paste-password").style.border = "1px solid red";
		} else {
			setPasswordArea(<></>);
			setLiked(data.liked);
			setPaste(data);
			...
			<div className="viewpaste">
				<div className="viewpaste_header">
					...
					<div className="user">
						<img
							src={
								paste.user.avatar
									? `${props.backend}/avatars/${paste.user.avatar}`
									: "/images/guest.jpg"
							}
							alt={paste.user.username}
						/>
						<Link to={`/accounts/view/${paste.user.username}`}>
							{paste.user.persianUsername || paste.user.username}
						</Link>
					</div>
				</div>
				<div className="content">
					<div className="comments">
						{menu === "newcomment" ? (
							<form className="new-comment" onSubmit={commentSubmit}>
								...
								<div className="submit">
									<button
										...
										onClick={async () => {
											if (comment) {
												const result = await props.fetchCreateComment(
													paste.paste.id,
													comment
												);
												if (result.id) {
							...
							<Scrollbar>
								...
								{comments.map((item) => {
									return (
										<div key={item.comment.id} className="comment">
											<div className="comment-header">
												<img
													src={
														item.user.avatar
															? `${props.backend}/avatars/${item.user.avatar}`
															: "/images/guest.jpg"
													}
													alt={item.user.username}
												/>
												<Link to={`/accounts/view/${item.user.username}`}>
													{item.user.persianUsername || item.user.username}
												</Link>
											</div>
				...
				<div
					className={`readme markdown-body${
						props.currentMode === "dark" ? "-dark" : ""
					}`}
					dangerouslySetInnerHTML={{
						__html: paste.paste.readme || "<p>فاقد فایل کمکی</p>",
					}}
				></div>
			</div>
		);
	}
	return <div className="view-paste-errors">{passwordArea}</div>;
};
